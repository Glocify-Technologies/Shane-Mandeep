...
open class AlamofireRequestBuilder<T>: RequestBuilder<T> {
    ...
    override open func execute(_ completion: @escaping (_ response: Response<T>?, _ error: Error?) -> Void) {
        ...
        let fileKeys = parameters == nil ? [] : parameters!.filter {
            if ($1 is NSURL || $1 is Array<NSURL>) {
                return true
            }else {
                return false
            }
                ...
                for (k, v) in self.parameters! {
                    ...
                    case let fileURL as URL:
                        if let mimeType = self.contentTypeForFormPart(fileURL: fileURL) {
                            ...
                            mpForm.append(fileURL, withName: k,fileName: fileURL.lastPathComponent, mimeType: "jpeg")
                        }
                    case let fileURLs as [URL]:
                        for i in 0..<fileURLs.count
                        {
                            if let mimeType = self.contentTypeForFormPart(fileURL: fileURLs[i]) {
                                mpForm.append(fileURLs[i], withName: k, fileName: fileURLs[i].lastPathComponent, mimeType: mimeType)
                            }
                            else {
                                let name = fileURLs[i].lastPathComponent
                                if name.contains(".pdf") {
                                    mpForm.append(fileURLs[i], withName: k,fileName: name, mimeType: "application/pdf")
                                }else{
                                    mpForm.append(fileURLs[i], withName: k,fileName: name, mimeType: "jpeg")
                                }
                                
                            }
                        }
                    case let SelectServices as NSMutableArray:
//                        for i in 0..<SelectServices.count
//                        {
                            if let data = self.json(from: SelectServices){
                                mpForm.append(data.data(using: String.Encoding.utf8)!, withName: k)
                            }
//                        }
