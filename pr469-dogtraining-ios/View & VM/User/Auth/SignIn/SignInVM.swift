...
class SignInVM: iBSBase, ObservableObject{
    //MARK: - PROPERITY
...
extension SignInVM: LoadingIndicatorDelegate {
    func signin(email: String, password: String, completion: @escaping (_ success: Bool) -> ()) {
        ...
        AuthAPI.authPostLogin(accept: "application/json", deviceId: ApplicationServiceProvider.shared.deviceId, deviceType: ApplicationServiceProvider.shared.deviceType, email: email, password: password, devicePushToken: "", completion: { data, error in
            // Stop Loading Indicator
            self.stopLoading()
...
//: MARK: - FormValidation
extension SignInVM {
    func formValidationSignIn(email: String, password: String) -> Bool {
        if email.isEmpty {
            alertTitle = .Failed
